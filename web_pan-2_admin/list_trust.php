  ...
  while($row=$ans->fetch_assoc())
   {
   	...
   	echo "<td>".$i."</td>"; 
		...
		echo "<td>".$row['post']."</td>";
		echo "<td>".$row['date']."</td>";
		echo "<td><img src='../web_pan-2_admin/trusti/".$row['image']. "'width='100' height='100'></td>";
	  $i=$i+1;?>
		<td><a href='?del=<?php echo $row['admin_id']; ?>' onclick="return confirm('Are You Sure?');">DELETE</a></td>
										<!--//outer-wp-->
									</div>
								</div>
								</div>
...
echo " </table>";
									 if(isset($_GET['del']))
									    {
                    					  $querya="DELETE FROM admin WHERE admin_id=".$_GET['del']."";
